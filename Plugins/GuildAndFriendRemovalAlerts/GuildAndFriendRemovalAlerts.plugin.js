...
function buildPlugin([BasePlugin, PluginApi]) {
	...
	(() => {
		...
		/* BetterDiscord v2: legacy BdApi.findModule* removed - shim via BdApi.Webpack (bracket names avoid replace_all corruption) */
		var GFRWebpack = (typeof BdApi !== "undefined" && BdApi.Webpack) ? BdApi.Webpack : {};
		function GFRFindByProps() {
			var a = arguments;
			var leg = BdApi && typeof BdApi["findModuleByProps"] === "function" && BdApi["findModuleByProps"];
			if (leg) return leg.apply(BdApi, a);
			if (GFRWebpack.getByProps) return GFRWebpack.getByProps.apply(GFRWebpack, a);
			if (GFRWebpack.getModule && GFRWebpack.Filters && typeof GFRWebpack.Filters.byKeys === "function")
				return GFRWebpack.getModule(GFRWebpack.Filters.byKeys.apply(GFRWebpack.Filters, a), { searchExports: true });
			return void 0;
		}
		function GFRFindModule(filter) {
			var leg = BdApi && typeof BdApi["findModule"] === "function" && BdApi["findModule"];
			if (leg) return leg.call(BdApi, filter);
			if (GFRWebpack.getModule) return GFRWebpack.getModule(filter, { searchExports: true });
			return void 0;
		}
		function GFRFindByDisplayName(name) {
			var leg = BdApi && typeof BdApi["findModuleByDisplayName"] === "function" && BdApi["findModuleByDisplayName"];
			if (leg) return leg.call(BdApi, name);
			if (GFRWebpack.getByDisplayName) return GFRWebpack.getByDisplayName(name);
			if (GFRWebpack.getModule && GFRWebpack.Filters && typeof GFRWebpack.Filters.byDisplayName === "function")
				return GFRWebpack.getModule(GFRWebpack.Filters.byDisplayName(name), { searchExports: true });
			return void 0;
		}
		function GFRFindByPrototypes() {
			var keys = Array.prototype.slice.call(arguments);
			var leg = BdApi && typeof BdApi["findModuleByPrototypes"] === "function" && BdApi["findModuleByPrototypes"];
			if (leg) return leg.apply(BdApi, keys);
			if (GFRWebpack.getModule && GFRWebpack.Filters && typeof GFRWebpack.Filters.byPrototypes === "function")
				return GFRWebpack.getModule(GFRWebpack.Filters.byPrototypes.apply(GFRWebpack.Filters, keys), { searchExports: true });
			if (GFRWebpack.getModule)
				return GFRWebpack.getModule(function (m) {
					if (!m || !m.prototype) return false;
					for (var i = 0; i < keys.length; i++) if (!(keys[i] in m.prototype)) return false;
					return true;
				}, { searchExports: true });
			return void 0;
		}
		function GFRFindAllModules(filter) {
			var leg = BdApi && typeof BdApi["findAllModules"] === "function" && BdApi["findAllModules"];
			if (leg) return leg.call(BdApi, filter);
			var out = [];
			try {
				var cache = GFRWebpack.modules;
				if (cache)
					for (var id in cache) {
						try {
							var ex = cache[id] && cache[id].exports;
							if (ex && filter(ex)) out.push(ex);
						} catch (e1) {}
					}
			} catch (e2) {}
			return out;
		...
		const Modules = {
			get 'react-spring'() {
				return ___createMemoize___(this, 'react-spring', () => GFRFindByProps('useSpring'))
			},
			'@discord/utils': {
				get 'joinClassNames'() {
					return ___createMemoize___(this, 'joinClassNames', () => GFRFindModule(e => e.toString().indexOf('return e.join(" ")') > 200))
				},
				get 'useForceUpdate'() {
					return ___createMemoize___(this, 'useForceUpdate', () => { const _x = GFRFindByProps('useForceUpdate'); return _x ? _x.useForceUpdate : void 0; })
				},
				get 'Logger'() {
					return ___createMemoize___(this, 'Logger', () => { const _x = GFRFindByProps('setLogFn'); return _x ? _x.default : void 0; })
				},
				get 'Navigation'() {
					return ___createMemoize___(this, 'Navigation', () => GFRFindByProps('replaceWith', 'currentRouteIsPeekView'))
				}
			},
			'@discord/components': {
				get 'Tooltip'() {
					return ___createMemoize___(this, 'Tooltip', () => GFRFindByDisplayName('Tooltip'))
				},
				get 'TooltipContainer'() {
					return ___createMemoize___(this, 'TooltipContainer', () => { const _x = GFRFindByProps('TooltipContainer'); return _x ? _x.TooltipContainer : void 0; })
				},
				get 'TextInput'() {
					return ___createMemoize___(this, 'TextInput', () => GFRFindByDisplayName('TextInput'))
				},
				get 'SlideIn'() {
					return ___createMemoize___(this, 'SlideIn', () => GFRFindByDisplayName('SlideIn'))
				},
				get 'SettingsNotice'() {
					return ___createMemoize___(this, 'SettingsNotice', () => GFRFindByDisplayName('SettingsNotice'))
				},
				get 'TransitionGroup'() {
					return ___createMemoize___(this, 'TransitionGroup', () => GFRFindByDisplayName('TransitionGroup'))
				},
				get 'Button'() {
					return ___createMemoize___(this, 'Button', () => GFRFindByProps('DropdownSizes'))
				},
				get 'Popout'() {
					return ___createMemoize___(this, 'Popout', () => GFRFindByDisplayName('Popout'))
				},
				get 'Flex'() {
					return ___createMemoize___(this, 'Flex', () => GFRFindByDisplayName('Flex'))
				},
				get 'Text'() {
					return ___createMemoize___(this, 'Text', () => GFRFindByDisplayName('Text'))
				},
				get 'Card'() {
					return ___createMemoize___(this, 'Card', () => GFRFindByDisplayName('Card'))
				}
			},
			'@discord/modules': {
				get 'Dispatcher'() {
					return ___createMemoize___(this, 'Dispatcher', () => GFRFindByProps('dispatch', 'subscribe'))
				},
				get 'ComponentDispatcher'() {
					return ___createMemoize___(this, 'ComponentDispatcher', () => { const _x = GFRFindByProps('ComponentDispatch'); return _x ? _x.ComponentDispatch : void 0; })
				},
				get 'EmojiUtils'() {
					return ___createMemoize___(this, 'EmojiUtils', () => GFRFindByProps('uploadEmoji'))
				},
				get 'PermissionUtils'() {
					return ___createMemoize___(this, 'PermissionUtils', () => GFRFindByProps('computePermissions', 'canManageUser'))
				},
				get 'DMUtils'() {
					return ___createMemoize___(this, 'DMUtils', () => GFRFindByProps('openPrivateChannel'))
				}
			},
			'@discord/stores': {
				get 'Messages'() {
					return ___createMemoize___(this, 'Messages', () => GFRFindByProps('getMessage', 'getMessages'))
				},
				get 'Channels'() {
					return ___createMemoize___(this, 'Channels', () => GFRFindByProps('getChannel', 'getDMFromUserId'))
				},
				get 'Guilds'() {
					return ___createMemoize___(this, 'Guilds', () => GFRFindByProps('getGuild'))
				},
				get 'SelectedGuilds'() {
					return ___createMemoize___(this, 'SelectedGuilds', () => GFRFindByProps('getGuildId', 'getLastSelectedGuildId'))
				},
				get 'SelectedChannels'() {
					return ___createMemoize___(this, 'SelectedChannels', () => GFRFindByProps('getChannelId', 'getLastSelectedChannelId'))
				},
				get 'Info'() {
					return ___createMemoize___(this, 'Info', () => GFRFindByProps('getSessionId'))
				},
				get 'Status'() {
					return ___createMemoize___(this, 'Status', () => GFRFindByProps('getStatus', 'getActivities', 'getState'))
				},
				get 'Users'() {
					return ___createMemoize___(this, 'Users', () => GFRFindByProps('getUser', 'getCurrentUser'))
				},
				get 'SettingsStore'() {
					return ___createMemoize___(this, 'SettingsStore', () => GFRFindByProps('afkTimeout', 'status'))
				},
				get 'UserProfile'() {
					return ___createMemoize___(this, 'UserProfile', () => GFRFindByProps('getUserProfile'))
				},
				get 'Members'() {
					return ___createMemoize___(this, 'Members', () => GFRFindByProps('getMember'))
				},
				get 'Activities'() {
					return ___createMemoize___(this, 'Activities', () => GFRFindByProps('getActivities'))
				},
				get 'Games'() {
					return ___createMemoize___(this, 'Games', () => GFRFindByProps('getGame', 'games'))
				},
				get 'Auth'() {
					return ___createMemoize___(this, 'Auth', () => GFRFindByProps('getId', 'isGuest'))
				},
				get 'TypingUsers'() {
					return ___createMemoize___(this, 'TypingUsers', () => GFRFindByProps('isTyping'))
				}
			},
			'@discord/actions': {
				get 'ProfileActions'() {
					return ___createMemoize___(this, 'ProfileActions', () => GFRFindByProps('fetchProfile'))
				},
				get 'GuildActions'() {
					return ___createMemoize___(this, 'GuildActions', () => GFRFindByProps('requestMembersById'))
				}
			},
			get '@discord/i18n'() {
				return ___createMemoize___(this, '@discord/i18n', () => GFRFindModule(m => m.Messages && m.Messages.CLOSE && typeof(m.getLocale) === 'function'))
			},
			get '@discord/constants'() {
				return ___createMemoize___(this, '@discord/constants', () => GFRFindByProps('API_HOST'))
			},
			get '@discord/contextmenu'() {
				return ___createMemoize___(this, '@discord/contextmenu', () => {
					const ctx = Object.assign({}, GFRFindByProps('openContextMenu'), GFRFindByProps('MenuItem'));
					ctx.Menu = ctx.default;
					return ctx;
				})
			},
			get '@discord/forms'() {
				return ___createMemoize___(this, '@discord/forms', () => GFRFindByProps('FormItem'))
			},
			get '@discord/scrollbars'() {
				return ___createMemoize___(this, '@discord/scrollbars', () => GFRFindByProps('ScrollerAuto'))
			},
			get '@discord/native'() {
				return ___createMemoize___(this, '@discord/native', () => GFRFindByProps('requireModule'))
			},
			get '@discord/flux'() {
				return ___createMemoize___(this, '@discord/flux', () => Object.assign({}, GFRFindByProps('useStateFromStores').default, GFRFindByProps('useStateFromStores')))
			},
			get '@discord/modal'() {
				return ___createMemoize___(this, '@discord/modal', () => Object.assign({}, GFRFindByProps('ModalRoot'), GFRFindByProps('openModal', 'closeAllModals')))
			},
			get '@discord/connections'() {
				return ___createMemoize___(this, '@discord/connections', () => GFRFindByProps('get', 'isSupported', 'map'))
			},
			get '@discord/sanitize'() {
				return ___createMemoize___(this, '@discord/sanitize', () => GFRFindByProps('stringify', 'parse', 'encode'))
			},
			get '@discord/icons'() {
				return ___createMemoize___(this, '@discord/icons', () => GFRFindAllModules(m => m.displayName && ~m.toString().indexOf('currentColor')).reduce((icons, icon) => (icons[icon.displayName] = icon, icons), {}))
			},
			'@discord/classes': {
				get 'Timestamp'() {
					return ___createMemoize___(this, 'Timestamp', () => GFRFindByPrototypes('toDate', 'month'))
				},
				get 'Message'() {
					return ___createMemoize___(this, 'Message', () => GFRFindByPrototypes('getReaction', 'isSystemDM'))
				},
				get 'User'() {
					return ___createMemoize___(this, 'User', () => GFRFindByPrototypes('tag'))
				},
				get 'Channel'() {
					return ___createMemoize___(this, 'Channel', () => GFRFindByPrototypes('isOwner', 'isCategory'))
				}
			}
		};
		var __webpack_modules__ = {
			...
			234: (__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
				...
				_defineProperty(Settings, "get", ((key, defaultValue) => (Settings.settings[key] != null ? Settings.settings[key] : defaultValue)));
				...
				function Item({
					...
					}, title), (description && description.length) ? external_BdApi_React_default().createElement("div", {
				...
				let HomeButtonModule = external_PluginApi_namespaceObject.WebpackModules.getByProps("HomeButton");
				if ((!HomeButtonModule || typeof HomeButtonModule.HomeButton !== "function") && typeof BdApi !== "undefined" && BdApi.Webpack) {
					const W = BdApi.Webpack;
					if (W.getModule) {
						const found = W.getModule((m) => m && typeof m.HomeButton === "function", { searchExports: true });
						if (found) HomeButtonModule = found;
					}
					if ((!HomeButtonModule || typeof HomeButtonModule.HomeButton !== "function") && W.getByKeys) {
						const hb = W.getByKeys("HomeButton");
						if (typeof hb === "function") HomeButtonModule = { HomeButton: hb };
					}
				}
				...
				class GuildAndFriendRemovalAlerts extends(external_BasePlugin_default()) {
					...
					onStart() {
						...
						if (HomeButtonModule && typeof HomeButtonModule.HomeButton === "function") {
							external_PluginApi_namespaceObject.Patcher.after(HomeButtonModule, "HomeButton", ((_, __, component) => {
							...
							external_PluginApi_namespaceObject.Logger.warn(
								"GuildAndFriendRemovalAlerts: HomeButton module not found; home button context menu patch skipped.",
							);
						}
					...
					openModal(guilds, friends, showClearButton = false) {
						...
						})), GuildAndFriendRemovalAlerts_React.createElement(modal_namespaceObject.ModalContent, props, ((guilds && guilds.length) || (friends && friends.length)) ? GuildAndFriendRemovalAlerts_React.createElement(GuildAndFriendRemovalAlerts_React.Fragment, null, showClearButton ? GuildAndFriendRemovalAlerts_React.createElement("div", {
							className: styles.Z.clearButton,
							onClick: clearLogs
						}, "Clear Logs") : null, (guilds && guilds.length) ? GuildAndFriendRemovalAlerts_React.createElement("div", {
						...
						}))))) : null, (friends && friends.length) ? GuildAndFriendRemovalAlerts_React.createElement("div", {
							className: styles.Z.itemContainer
						}, GuildAndFriendRemovalAlerts_React.createElement("div", {
							className: styles.Z.title
	...
	return (PluginExports && PluginExports.__esModule) ? PluginExports.default : PluginExports;
}
			...
			BdApi.showConfirmationModal(
				...
				[`The library plugin needed for ${config.info.name} is missing. Please click Download to install it.`], {
					...
					onConfirm: () => {
						...
						const fallback = "https://betterdiscord.net/ghdl?url=https://raw.githubusercontent.com/rauenzi/BDPluginLibrary/master/release/0PluginLibrary.plugin.js";
						if (BdApi.Net && typeof BdApi.Net.fetch === "function") {
							BdApi.Net.fetch(url)
								.then((r) => (r && r.ok ? r.text() : Promise.reject()))
								.then((body) => new Promise((resolve, reject) => fs.writeFile(dest, body, (err) => (err ? reject(err) : resolve()))))
								.catch(() => shell.openExternal(fallback));
						} else {
							require("request").get(url, async (error, _response, body) => {
								if (error) return shell.openExternal(fallback);
								await new Promise((r) => fs.writeFile(dest, body, r));
							});
						}
					}
				}
