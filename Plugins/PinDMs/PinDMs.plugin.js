	...
	return !window.BDFDB_Global || (!window.BDFDB_Global.loaded && !window.BDFDB_Global.started) ? class {
		...
		return class PinDMs extends Plugin {
			onLoad () {
				this.defaults = {
					general: {
						pinIcon:		{value: true, 						description: "Shows a little 'Pin' Icon for pinned DMs in the Server List"},
				...
				this.css = `
					...
					#app-mount ${BDFDB.dotCN.dmchannelheadercontainer} {
						margin-top: 10px;
					}
			...
			processPrivateChannelsList (e) {
				const DMItemComp = BDFDB.LibraryComponents.PrivateChannelItems && BDFDB.LibraryComponents.PrivateChannelItems.DirectMessage;
				let categories = this.sortAndUpdateCategories("channelList", true);
				...
				if (!e.returnvalue) {
					...
					if (!this._pindmsCompatModeLogged) {
						...
						try {
							console.warn("[PinDMs] Compatibility mode active: using safe section headers without custom DM row renderer.");
						}
						catch (_err) {}
					}
