...
module.exports = class DoubleClickToEdit {
	...
	start() {
		try {
			//Classes
			let _selMod = Webpack.getModule(Filters.byKeys("message", "selected"));
			if (!_selMod?.selected)
				_selMod = Webpack.getModule(
					(m) => m && typeof m === "object" && typeof m.selected === "string" && typeof m.message === "string",
					{ searchExports: true },
				);
			if (!_selMod?.selected)
				_selMod = Webpack.getModule(Filters.byKeys("cozyMessage", "compactMessage"), { searchExports: true });
			if (!_selMod?.selected)
				_selMod = Webpack.getModule(
					(m) =>
						m &&
						typeof m === "object" &&
						typeof m.selected === "string" &&
						(m.message || m.cozyMessage || m.compactMessage),
					{ searchExports: true },
				);
			let _wrapMod = Webpack.getModule(Filters.byKeys("empty", "messagesWrapper"));
			if (!_wrapMod?.messagesWrapper)
				_wrapMod = Webpack.getModule(Filters.byKeys("messagesWrapper"), { searchExports: true });
			if (!_wrapMod?.messagesWrapper)
				_wrapMod = Webpack.getModule(
					(m) =>
						m &&
						typeof m === "object" &&
						typeof m.messagesWrapper === "string" &&
						(m.scroller || m.message || m.chat),
					{ searchExports: true },
				);
			if (_selMod && !_selMod.selected && typeof _selMod.message === "string") {
				const altSel = Webpack.getModule(
					(m) =>
						m &&
						m !== _selMod &&
						typeof m === "object" &&
						typeof m.selected === "string" &&
						m.message === _selMod.message,
					{ searchExports: true },
				);
				if (altSel?.selected) _selMod = altSel;
			}
			if (!_selMod?.selected || !_wrapMod?.messagesWrapper) {
				console.warn("DoubleClickToEdit: message CSS modules (message/selected/messagesWrapper) not found; plugin disabled for this Discord build.");
				return;
			}
			this.selectedClass = _selMod.selected;
			this.messagesWrapper = _wrapMod.messagesWrapper;
