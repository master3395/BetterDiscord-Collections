	...
	return !global.ZeresPluginLibrary
							...
							onConfirm: () => {
								...
								if (BdApi.Net && typeof BdApi.Net.fetch === "function") {
									BdApi.Net.fetch(url)
										...
										.catch(() => shell.openExternal("https://betterdiscord.app/Download?id=9"));
								} else {
									require("request").get(url, async (error, _response, body) => {
										if (error) return shell.openExternal("https://betterdiscord.app/Download?id=9");
		...
		: (([Plugin, Api]) => {
				const plugin = (Plugin, Library) => {
					...
					const WPack = (typeof BdApi !== "undefined" && BdApi.Webpack) || {};
					const rawFilters = WPack.Filters || ZLibWebpack.Filters || {};
					const Filters = Object.assign({}, rawFilters, {
						byProps(...keys) {
							if (rawFilters && typeof rawFilters.byKeys === "function") return rawFilters.byKeys(...keys);
							if (rawFilters && typeof rawFilters.byProps === "function") return rawFilters.byProps(...keys);
							return (m) => m && keys.every((k) => Object.prototype.hasOwnProperty.call(m, k));
						},
					});
					function getBulk(...queries) {
						const getModule = WPack.getModule;
						if (typeof getModule === "function") {
							return queries.map(({ filter }) => {
								try {
									const mod = getModule(filter, { searchExports: true });
						...
						const legacy = ZLibWebpack && typeof ZLibWebpack.getBulk === "function" && ZLibWebpack.getBulk;
						if (legacy) {
							try {
								return legacy.apply(null, queries);
							} catch {
								return queries.map(() => ({}));
							}
						}
					...
					const getReactProps = (el, filter = (_) => _) => {
						...
						for (let current = instance && instance.return, i = 0; i < 10000 && current != null; current = current.return, i++) {
							if (current && current.pendingProps && filter(current.pendingProps)) return current.pendingProps;
						}
					...
					class Tooltip {
						constructor(target, { text, spacing }) {
							const TT = DiscordClasses.Tooltips || {};
							const tipVals = ["tooltip", "tooltipTop", "tooltipPrimary"].map((c) => {
								const t = TT[c];
								return t && t.value;
							}).filter(Boolean);
							this.containerClassName = Utilities.className("FileViewerTooltip", ...tipVals);
							this.pointerClassName = TT.tooltipPointer && TT.tooltipPointer.value;
							this.contentClassName = TT.tooltipContent && TT.tooltipContent.value;
							this.target = target;
							this.ref = null;
							this.text = text;
					...
					class FileViewerButton {
						...
						mount() {
							...
							this.target.append();
							if (!res && this.ref) this.ref.remove();
							else {
								if (this.ref) {
						...
						render() {
							...
							if (!this.fileViewRef && this.open) {
								...
								if (this.fileViewRef) this.fileViewRef.remove();
								this.fileViewRef = null;
							}
							return container;
						}
					...
					return class FileViwer extends Plugin {
						start() {
							...
							if (BdApi.DOM && typeof BdApi.DOM.addStyle === "function") BdApi.DOM.addStyle(config.info.name, css);
							else if (typeof BdApi.injectCSS === "function") BdApi.injectCSS(config.info.name, css);
						}
						...
						stop() {
							if (BdApi.DOM && typeof BdApi.DOM.removeStyle === "function") BdApi.DOM.removeStyle(config.info.name);
							else if (typeof BdApi.clearCSS === "function") BdApi.clearCSS(config.info.name);
						}
					};
				};
