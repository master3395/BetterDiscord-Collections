...
function _cpFindByProps() {
	const W = typeof BdApi !== "undefined" && BdApi.Webpack;
	const leg = BdApi && typeof BdApi["findModuleByProps"] === "function" && BdApi["findModuleByProps"];
	if (leg) return leg.apply(BdApi, arguments);
	if (W && W.getByProps) return W.getByProps.apply(W, arguments);
	if (W && W.getModule && W.Filters && typeof W.Filters.byKeys === "function")
		return W.getModule(W.Filters.byKeys.apply(W.Filters, arguments), { searchExports: true });
	return void 0;
}
function _cpFindModule(filter) {
	const W = typeof BdApi !== "undefined" && BdApi.Webpack;
	const leg = BdApi && typeof BdApi["findModule"] === "function" && BdApi["findModule"];
	if (leg) return leg.call(BdApi, filter);
	if (W && typeof W.getModule === "function") return W.getModule(filter, { searchExports: true });
	return void 0;
}
function _cpDataLoad(pluginId, key) {
	if (BdApi.Data && typeof BdApi.Data.load === "function") return BdApi.Data.load(pluginId, key);
	if (typeof BdApi.loadData === "function") return BdApi.loadData(pluginId, key);
	return void 0;
}
function _cpDataSave(pluginId, key, value) {
	if (BdApi.Data && typeof BdApi.Data.save === "function") return BdApi.Data.save(pluginId, key, value);
	if (typeof BdApi.saveData === "function") return BdApi.saveData(pluginId, key, value);
}
...
module.exports = class ChannelPermissions {
		...
		if (!global.ZeresPluginLibrary)
			BdApi.showConfirmationModal("Library Missing", `The library plugin needed for ${this.getName()} is missing. Please click Download Now to install it.`, {
				...
				onConfirm: () => {
					...
					const fallback = "https://raw.githubusercontent.com/rauenzi/BDPluginLibrary/master/release/0PluginLibrary.plugin.js";
					const dest = path.join(BdApi.Plugins.folder, "0PluginLibrary.plugin.js");
					if (BdApi.Net && typeof BdApi.Net.fetch === "function") {
						BdApi.Net.fetch(url)
							.then((r) => (r && r.ok ? r.text() : Promise.reject(new Error(String(r && r.status)))))
							.then(
								(body) =>
									new Promise((resolve, reject) => {
										fs.writeFile(dest, body, (err) => (err ? reject(err) : resolve()));
									}),
							)
							.then(() => {
								if (BdApi.UI && typeof BdApi.UI.showToast === "function")
									BdApi.UI.showToast("0PluginLibrary downloaded.", { type: "success" });
							})
	...
	start() {
		try {
			//Inject our styles (BD v2: DOM.addStyle / removeStyle)
			const _cpStyle = `
...
.${config.constants.topicClass} > span > svg {
	transform-scale: 10%;
			...
			if (BdApi.DOM && typeof BdApi.DOM.addStyle === "function") BdApi.DOM.addStyle(config.constants.cssStyle, _cpStyle);
			else if (typeof BdApi.injectCSS === "function") BdApi.injectCSS(config.constants.cssStyle, _cpStyle);

			//Modules for the settings
			const _ti = _cpFindModule((m) => m?.default?.displayName === "TextInput");
			this.TextInput = _ti && _ti.default;
			this.FormStore = _cpFindByProps("FormItem");
			//Load any settings we have
			this.openingPopoutDelay = _cpDataLoad(config.info.id, "openingPopoutDelay") ?? config.constants.popoutDelay;
			this.closingPopoutDelay = _cpDataLoad(config.info.id, "closingPopoutDelay") ?? config.constants.popoutDelay;

			//Lose/single classes
			const _cd1 = _cpFindByProps("actionIcon", "containerDefault");
			const _cd2 = _cpFindByProps("spaceBeforeCategory", "containerDefault");
			this.containerDefault = _cd1 && _cd1.containerDefault;
			this.categoryContainer = _cd2 && _cd2.containerDefault;

			//Class collections
			this.roleClasses = _cpFindModule((m) => m.role && m.roleName);
			this.roleCircleClasses = _cpFindModule((m) => m.roleCircle);
			this.roleListClasses = _cpFindByProps("rolesList");
			this.popoutRootClasses = _cpFindByProps("container", "activity");
			this.popoutBodyClasses = _cpFindByProps("thin", "scrollerBase");
			this.popoutChannelActivityClasses = _cpFindByProps("channelActivityContainer", "activity");

			//Permissions
			this.PermissionStore = _cpFindByProps("Permissions", "ActivityTypes");
			const _permHas = _cpFindByProps("deserialize", "invert", "has");
			this.hasPermission = _permHas && _permHas.has;

			//Cache the function, makes it easier.
			//We can't make these methods cleanly because that would make a `findModule` call.
			const _gg = _cpFindByProps("getGuild", "getGuilds");
			const _gc = _cpFindByProps("getChannel", "getDMFromUserId");
			const _gm = _cpFindByProps("getMember", "getMembers");
			this.getGuild = _gg && _gg.getGuild;
			this.getChannel = _gc && _gc.getChannel;
			this.getMember = _gm && _gm.getMember;

			//Set local store and get the functions we need.
			const UserStore = _cpFindByProps("getUser", "getUsers");
			this.getUser = UserStore && UserStore.getUser;
			this.getCurrentUser = UserStore && UserStore.getCurrentUser;

			let _ss = _cpFindByProps("useStateFromStoresArray");
			if (!_ss || typeof _ss.useStateFromStoresArray !== "function") {
				const W = typeof BdApi !== "undefined" && BdApi.Webpack;
				if (W && typeof W.getModule === "function") {
					const found = W.getModule(
						(m) => m && typeof m.useStateFromStoresArray === "function",
						{ searchExports: true },
					);
					if (found) _ss = found;
				}
			}
			if ((!_ss || typeof _ss.useStateFromStoresArray !== "function") && BdApi.Webpack?.getByKeys) {
				const u = BdApi.Webpack.getByKeys("useStateFromStoresArray");
				if (typeof u === "function") _ss = { useStateFromStoresArray: u };
			}
			this.StateStore = _ss;

			//Get our popout module we will patch and use
			this.ActiveThreadsPopoutModule = _cpFindModule((m) => m?.default?.displayName === "ActiveThreadsPopout");
			this.PopoutModule = _cpFindModule((m) => m?.default?.displayName === "Popout");

			//Store color converter (hex -> rgb) and d
			const _hex = _cpFindByProps("getDarkness", "isValidHex");
			this.hex2rgb = _hex && _hex.hex2rgb;
			const _txt = _cpFindModule((m) => m.astParserFor && m.parse);
			this.text2DiscordParser = _txt && _txt.parse;
			...
			React.createElement(this.FormStore.FormItem, {
				...
				React.createElement(this.TextInput, {
					...
					onChange: (e) => {
						...
						_cpDataSave(config.info.id, "openingPopoutDelay", e);
			...
			React.createElement(this.FormStore.FormItem, {
				...
				React.createElement(this.TextInput, {
					...
					onChange: (e) => {
						...
						_cpDataSave(config.info.id, "closingPopoutDelay", e);
	...
	stop() {
		...
		if (BdApi.DOM && typeof BdApi.DOM.removeStyle === "function") BdApi.DOM.removeStyle(config.constants.cssStyle);
		else if (typeof BdApi.clearCSS === "function") BdApi.clearCSS(config.constants.cssStyle);
	}

	patchThreadPopout() {
		if (!this.ActiveThreadsPopoutModule?.default) return;
		const useStateFromStoresArray =
			this.StateStore && typeof this.StateStore.useStateFromStoresArray === "function"
				? this.StateStore.useStateFromStoresArray
				: null;
		if (!useStateFromStoresArray) {
			try {
				console.warn("[ChannelPermissions] useStateFromStoresArray not found; thread popout patch skipped.");
			} catch (_e) {}
			return;
		}
		const ThreadsStore = _cpFindByProps("getActiveUnjoinedThreadsForParent");
		const GuildPermissions = _cpFindByProps("getGuildPermissions");
		if (!ThreadsStore || !GuildPermissions || !this.PermissionStore?.Permissions) {
			try {
				console.warn("[ChannelPermissions] Thread popout stores incomplete; thread popout patch skipped.");
			} catch (_e2) {}
			return;
		}
		const permissionTypes = this.PermissionStore.Permissions;
		const self = this;
	...
	patchVoiceActivities() {
		//Get the module
		const VoiceChannelActivities = _cpFindModule((m) => m?.default?.displayName === "VoiceChannelActivities");
		if (!VoiceChannelActivities?.default) {
			try {
				console.warn("[ChannelPermissions] VoiceChannelActivities not found; voice activities patch skipped.");
			} catch (_e) {}
			return;
		}
