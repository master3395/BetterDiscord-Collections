			...
			runQueue () {
				if (!requestQueue.id) {
					...
					if (id) {
						...
						try {
							const fetchProfile = BDFDB.LibraryModules.UserProfileUtils && BDFDB.LibraryModules.UserProfileUtils.fetchProfile;
							if (typeof fetchProfile === "function") {
								const pr = fetchProfile.call(BDFDB.LibraryModules.UserProfileUtils, id);
								if (pr && typeof pr.catch === "function")
									pr.catch(() => {
										requestQueue.id = null;
										BDFDB.TimeUtils.timeout(_ => _this.runQueue(), 500);
									});
							} else {
								requestQueue.id = null;
							}
